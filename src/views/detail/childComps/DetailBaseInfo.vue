<template>
  <div v-if="Object.keys(goodsInfo).length !== 0" class="base-info">
    ...
    <div class="info-other">
      <span v-if="goodsInfo.columns[0]">{{goodsInfo.columns[0]}}</span>
      <span v-if="goodsInfo.columns[1]">{{goodsInfo.columns[1]}}</span>

      <span v-if="goodsInfo.services.length !==0">{{goodsInfo.services[goodsInfo.services.length - 1].name}}</span>
    </div>
